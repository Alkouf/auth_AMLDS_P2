"""
Silhouette Analysis using 10% of random selected  instances.
"""
print(__doc__)
...
X_rej, X, y_train, y_test = train_test_split(instances, range(len(instances)), test_size=0.1, random_state=42)
text = ""
...
plt.suptitle("Silhouette analysis for KMeans clustering on 10% of instances", fontsize=10, fontweight='bold')
...
with open("silhouette_analysis_results.txt", "wb") as f:
    f.write(text)
    f.close()
plt.show()
